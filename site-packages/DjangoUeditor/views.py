...
#保存上传的文件
def save_upload_file(PostFile,FileName):

	#write to sae storage 
	c=Connection(accesskey=const.ACCESS_KEY ,secretkey=const.SECRET_KEY,account=const.APP_NAME)
	bucket=c.get_bucket('blog')
	bucket.put_object(FileName,PostFile.read())
	return u"SUCCESS"
...
def get_files(root_path,cur_path,allow_types=[]):
	...
	for i in bucket.list():
		files.append({
			'url':bucket.generate_url(i.name),
...
def UploadFile(request):
	...
	if state=="SUCCESS":
		if action=="uploadscrawl":
			...
			state=save_upload_file(file,OutputFile)

	#返回数据
	return_info = {
		'url': urllib.basejoin(USettings.gSettings.MEDIA_URL , OutputFile) ,                # 保存后的文件名称
...
def get_output_path(request,path_format,path_format_var):
	...
	OutputPath=USettings.gSettings.MEDIA_ROOT
